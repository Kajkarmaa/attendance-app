...
export default function HomeScreen() {
    ...
    const [selectedPayrollEmployee, setSelectedPayrollEmployee] =
    ...
    const stats = useMemo(() => {
        ...
        return [
            {
                label: "Staff Heads",
                value: formatValue(dailySummary?.totalEmployees),
                color: "#60A5FA",
            },
            {
                label: "Checked In",
                value: formatValue(dailySummary?.totalCheckedIn),
                color: "#34D399",
            },
            {
                label: "Checked Out",
                value: formatValue(dailySummary?.totalCheckedOut),
                color: "#0EA5E9",
            },
            {
                label: "Staff Present",
                value: formatValue(dailySummary?.staffPresent),
                color: "#D4A537",
            },
            {
                label: "Staff Absent",
                value: formatValue(dailySummary?.staffAbsent),
                color: "#F472B6",
            },
            {
                label: "Staff Leave",
                value: formatValue(dailySummary?.staffOnLeave),
                color: "#A78BFA",
            },
            {
    ...
    const handleGeneratePayroll = async () => {
        if (!selectedPayrollEmployee?.employeeId) {
            setPayrollMessage({
                text: "Missing employee identifier.",
                tone: "error",
            });
            return;
        }
    ...
    const handleConvertToEmployee = async () => {
        ...
        if (!designation || !departmentValue) {
            setConvertMessage({
                text: "Please fill designation and department.",
                tone: "error",
            });
            return;
        }
        ...
        if (!Number.isFinite(salaryNumber) || salaryNumber <= 0) {
            setConvertMessage({
                text: "Enter a valid salary amount.",
                tone: "error",
            });
            return;
        }
                ...
                >
                    ...
                    >
                        ...
                        <Text style={styles.payrollSubtitle}>
                            {selectedPayrollEmployee?.userId?.name ||
                                selectedPayrollEmployee?.employeeId ||
                                "Select employee"}
                        </Text>
                        {selectedPayrollEmployee?.employeeId && (
                            <Text style={styles.payrollMeta}>
                                ID: {selectedPayrollEmployee.employeeId} •{" "}
                                {selectedPayrollEmployee.department ||
                                    "Department"}
                            </Text>
                        )}
                        ...
                        <Text style={styles.payrollHint}>
                            Payroll runs immediately for the selected month.
                        </Text>
                    </Pressable>
                </Pressable>
            </Modal>
                ...
                >
                    ...
                    >
                        ...
                        <Text style={styles.convertSubtitle}>
                            {selectedPendingUser?.name || "Select pending user"}
                        </Text>
                        {selectedPendingUser?.email && (
                            <Text style={styles.convertMeta}>
                                {selectedPendingUser.email}
                            </Text>
                        )}
                        <Text style={styles.convertDescription}>
                            Assign designation, department, and salary to
                            finalize onboarding.
                        </Text>
                        ...
                        <View style={styles.convertActions}>
                            ...
                            >
                                <Text style={styles.convertCancelText}>
                                    Cancel
                                </Text>
                            </Pressable>
                            <Pressable
                                style={[
                                    styles.convertSubmit,
                                    convertLoading &&
                                        styles.convertSubmitDisabled,
                                ]}
                            ...
                            >
                                {convertLoading ? (
                                    ...
                                    <Text style={styles.convertSubmitText}>
                                        Convert
                                    </Text>
                                )}
                            </Pressable>
                        ...
                        <Text style={styles.convertHint}>
                            Converted users appear instantly in the employee
                            tab.
                        </Text>
                    </Pressable>
                </Pressable>
